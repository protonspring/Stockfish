...
namespace {
  ...
  int FutilityMoveCounts[16]; // [improving][depth]
  ...
  inline int futility_move_count(bool imp, int d) {
    return FutilityMoveCounts[d] * (1 + imp);
  }
...
void Search::init() {
  ...
  for (int d = 0; d < 16; ++d)
      FutilityMoveCounts[d] = int(5 + 9 * d * d / 10) / 2;
}
