         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              alpha = std::max<int>(previousScore - delta,-VALUE_INFINITE);
              beta  = std::min<int>(previousScore + delta, VALUE_INFINITE);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        Value raisedBeta = std::min<int>(beta + 189 - 45 * improving, VALUE_INFINITE);
