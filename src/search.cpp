...
void Thread::search() {
  ...
  int ct = int(Options["Contempt"]) * PieceValue[EG][PAWN] / 100; // From centipawns
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_MATED_IN_MAX_PLY)
      {
          ...
          if (   !captureOrPromotion
              && !givesCheck
              && (!pos.advanced_pawn_push(move) || pos.non_pawn_material(~us) > PieceValue[MG][BISHOP]))
          {
      ...
      if (    depth >= 3 * ONE_PLY
          ...
          && (  !captureOrPromotion
              || moveCountPruning
              || ss->staticEval + PieceValue[EG][type_of(pos.captured_piece())] <= alpha
              || cutNode))
      {
    ...
    else if (bestMove)
    {
        ...
        if (!pos.capture_or_promotion(bestMove))
            update_quiet_stats(pos, ss, bestMove, quietsSearched, quietCount,
                               stat_bonus(depth + (bestValue > beta + PieceValue[MG][PAWN] ? ONE_PLY : DEPTH_ZERO)));
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
          ...
          && !pos.advanced_pawn_push(move))
      {
          ...
          futilityValue = futilityBase + PieceValue[EG][type_of(pos.piece_on(to_sq(move)))];
  ...
  Move Skill::pick_best(size_t multiPV) {
    ...
    int delta = std::min(topScore - rootMoves[multiPV - 1].score, PieceValue[MG][PAWN]);
