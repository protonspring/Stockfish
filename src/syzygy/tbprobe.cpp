...
constexpr Value16 WDL_to_value[] = {
   -VALUE_MATE16 + MAX_PLY + 1,
    VALUE_DRAW16 - 2,
    VALUE_DRAW16,
    VALUE_DRAW16 + 2,
    VALUE_MATE16 - MAX_PLY - 1
...
bool Tablebases::root_probe(Position& pos, Search::RootMoves& rootMoves) {
    ...
    for (auto& m : rootMoves)
    {
        ...
        m.tbScore =  r >= bound ? VALUE_MATE16 - MAX_PLY - 1
                   : r >  0     ? Value16((std::max( 3, r - 800) * int(PawnValueEg16)) / 200)
                   : r == 0     ? VALUE_DRAW16
                   : r > -bound ? Value16((std::min(-3, r + 800) * int(PawnValueEg16)) / 200)
                   :             -VALUE_MATE16 + MAX_PLY + 1;
    }
