...
namespace {
  ...
  // Connected pawn bonus by opposed, phalanx, supported and rank
  ...
  Score evaluate(const Position& pos, Pawns::Entry* e) {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (supported | phalanx)
            score += Connected[opposed][bool(phalanx)][bool(supported)][relative_rank(Us, s)];
...
void init() {
  ...
  {
      int v = 17 * support;
      v += (Seed[r] + (phalanx * (Seed[r + 1] - Seed[r]) / 2)) >> opposed;
