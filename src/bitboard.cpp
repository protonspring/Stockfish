...
Bitboard PathBB[SQUARE_NB][SQUARE_NB];
...
void Bitboards::init() {
  ...
  for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1)
      ...
      {
          //Path between the squares
          Square x = s1;
          while (x != s2)
          {
              if (file_of(x) < file_of(s2)) x += EAST;
              if (file_of(x) > file_of(s2)) x += WEST;
              if (rank_of(x) > rank_of(s2)) x += SOUTH;
              if (rank_of(x) < rank_of(s2)) x += NORTH;

              if (x != s2) PathBB[s1][s2] |= x;
          }
      }
