...
void init() {
  ...
  for (File f = FILE_A; f <= FILE_H; ++f) {
     ...
     PassedRank[f] = make_score(Value(PassedRankMG[f]),Value(PassedRankEG[f]));
     //PassedRank[f] = Rank(f) < RANK_2 ? make_score(0,0) : make_score(Value(PassedRankMG[f]),(f-1)*(f-1)*(f-1)+6*(f-1)*(f-1)-5*(f-1)+10);
  }
}
