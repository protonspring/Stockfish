  ...
  Score Evaluation<T>::passed() const {
    ...
    while (b)
    {
        ...
        if (r > RANK_3)
        {
            ...
            bonus += make_score(0, (  (7 * distance(pos.square<KING>(Them), blockSq)
                                      -3 * distance(pos.square<KING>(  Us), blockSq)) * w / 2));

            // If blockSq is not the queening square then consider also a second push
            if (r != RANK_7)
                bonus -= make_score(0, 6 * distance(pos.square<KING>(Us), blockSq + Up) * w / 8);
