...
#undef S
  ...
  void Evaluation<T>::initialize() {
    ...
    constexpr Bitboard LowRanks = (Us == WHITE ? Rank2BB | Rank3BB | Rank4BB
    ...
    // Find our pawns that on the first three ranks
    Bitboard b = pos.pieces(Us, PAWN) & LowRanks;
  ...
  Score Evaluation<T>::passed() const {
    ...
    while (b)
    {
        ...
        if (r > RANK_3)
        {
            ...
            // More penalty if blockSq is not the queening square
            bonus += make_score(0, (  king_proximity(Them, blockSq)  * 5
                   - ((r != RANK_7) + king_proximity(Us,   blockSq)) * 2) * w);
