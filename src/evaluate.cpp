  ...
  Score Evaluation<T>::pieces() {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (Pt == BISHOP)
            {
                // Penalty for pawns ahead of the bishop and on same color squares
                // Bonus for pawns that are behind and on same color squares
                Bitboard bps = (s & DarkSquares) ? pos.pieces(Us,PAWN) & DarkSquares : pos.pieces(Us,PAWN) & ~DarkSquares;

                score += BishopPawns * (popcount(bps & forward_ranks_bb(Them,s))
                                       -popcount(bps & forward_ranks_bb(Us,s)));
