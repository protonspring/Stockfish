...
#define S(mg, eg) make_score(mg, eg)
  ...
  Score BishopOurPawns     = S(  8, 12);
  Score BishopTheirPawns   = S(  8, 12);
  ...
  TUNE(BishopOurPawns,BishopTheirPawns);
  ...
  Score Evaluation<T>::pieces() {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            if (Pt == BISHOP)
            {
                //if one bishop, bonus for enemy pawns on that color and penalty
                //for our pawns on that color.
                if (pos.count<BISHOP>(Us) == 1)
                {
                  score += BishopTheirPawns * pe->pawns_on_same_color_squares(Them, s);
                  score -= BishopOurPawns * pe->pawns_on_same_color_squares(Us, s);
                }
