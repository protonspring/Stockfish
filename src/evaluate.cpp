  ...
  Score Evaluation<T>::passed() const {
    ...
    while (b)
    {
        ...
        if (r > RANK_3)
        {
            ...
            bonus += make_score(0, (  distance(pos.square<KING>(Them), blockSq) * 4
                                    - distance(pos.square<KING>(Us),   blockSq) * 2) * w);

            // If blockSq is not the queening square then consider also a second push
            if (r != RANK_7)
                bonus -= make_score(0, distance(pos.square<KING>(Us), blockSq + Up) * w);
