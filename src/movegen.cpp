...
namespace {
  ...
  ExtMove* generate_pawn_moves(const Position& pos, ExtMove* moveList, Bitboard target) {
    ...
    constexpr Direction UpRight  = Up(Us) + Right(Us);
    constexpr Direction UpLeft   = Up(Us) + Left(Us);
    ...
    Bitboard enemies = (Type == EVASIONS ? pos.pieces(Them(Us)) & target:
                        Type == CAPTURES ? target : pos.pieces(Them(Us)));
    ...
    if (Type != CAPTURES)
    {
        ...
        {
            Square ksq = pos.square<KING>(Them(Us));

            b1 &= pos.attacks_from<PAWN>(ksq, Them(Us));
            b2 &= pos.attacks_from<PAWN>(ksq, Them(Us));

            // Add pawn pushes which give discovered check. This is possible only
            // if the pawn is not on the same file as the enemy king, because we
            // don't generate captures. Note that a possible discovery check
            // promotion has been already generated amongst the captures.
            Bitboard dcCandidateQuiets = pos.blockers_for_king(Them(Us)) & pawnsNotOn7;
    ...
    if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS)
    {
        ...
        if (pos.ep_square() != SQ_NONE)
        {
            ...
            b1 = pawnsNotOn7 & pos.attacks_from<PAWN>(pos.ep_square(), Them(Us));
